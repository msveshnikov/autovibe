...
const RANKS = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']; // T=10, J=Jack, Q=Queen, K=King, A=Ace
const RANK_VALUES = {
    2: 2,
    3: 3,
    4: 4,
...
function evaluateHand(hand) {
    ...
    if (counts[0] === 3) {
        ...
        const kickers = values.filter((v) => v !== threeKindValue).sort((a, b) => b - a);
        return {
...
function showdown() {
    ...
    let message = `Player has: ${playerResult.name}. Dealer has: ${dealerResult.name}. `;
...
function updateUI(showDealerFullHand = false) {
    ...
    playerHandDiv.innerHTML = formatCards(playerHand, true, gamePhase === 'player-draw-phase');
    ...
    if (!drawButton) {
        ...
        drawButton.style.display = gamePhase === 'player-draw-phase' ? 'inline-block' : 'none';
    }
...
function formatCards(hand, showAll = true, isPlayerTurn = false) {
    ...
    hand.forEach((card, index) => {
        ...
        if (showAll) {
            html += `
        <div class="card ${colorClass}" ${isPlayerTurn ? `data-card-index="${index}"` : ''}>
...
if (drawBtnElement) {
    ...
    console.warn("Draw button element with id 'draw-button' not found in HTML. Creating one.");
